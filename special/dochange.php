...
$sSid = $_POST['saleId'];
$res = mysql_query("SELECT ProdId FROM SpecialSale WHERE SpslId=$sSid");
if (!($row = mysql_fetch_array($res))) $errmsg .= 'Invalid sale. ';
...
if (!$errmsg) {
  mysql_query("UPDATE SpecialSale SET ProdId='$sPid', SalePrice='$sPrice', StartDate='$sSdate', EndDate='$sEdate'
    WHERE SpslId=$sSid");
  mysql_close($con);
  echo '<script type="text/javascript">';
  echo 'alert("Sale content changed!")';
  echo '</script>';
